...
def sail_model_init (x_0 : Unit) : SailM Unit := do
  ...
  writeReg senvcfg (← (legalize_senvcfg (Mk_SEnvcfg (zeros (n := 64)))
      (zeros (n := ((2 ^i 3) *i 8)))))
  writeReg mvendorid (to_bits_unsafe (l := 32) (0 : Int))
  writeReg mimpid (to_bits_unsafe (l := xlen) (0 : Int))
  writeReg marchid (to_bits_unsafe (l := xlen) (0 : Int))
  writeReg mhartid (to_bits_unsafe (l := xlen) (0 : Int))
